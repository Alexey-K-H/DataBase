...
public class AdminDB extends UserMod{
    public AdminDB(String nameServer, Properties properties, String url) {
    ...
    public void openSecurityCheckWindow() {
        ...
        this.setTitle("Вход как администратор БД");
        ...
        JLabel info = new JLabel("<html>Введите пароль<br>администратора БД</html>");
        ...
        confirm.addActionListener(e -> {
            ...
            try {
                if(checkSecurity(pwd.toString(), getProperties().getProperty("password"))){
                    ...
                    MainWindow mainWindow = new MainWindow(connection, getNameServer(), UserMods.ADMINISTRATOR_DB, debug);
                    mainWindow.run();
                }
                else {
                    throw new SQLException("Неверный пароль администратора БД!");
                }
