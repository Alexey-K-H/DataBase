...
public class WorkersInsert extends JDialog implements InsertFrame {
    private final TableController tableController;
    private ArrayList<String> currValues;
    private final DefaultTableModel tableModel;

    public WorkersInsert(TableController tableController, DefaultTableModel tableModel) {
    ...
    public void openInsertWindow() {
        ...
        this.setTitle("Добавление нового работника");
        ...
        JLabel info = new JLabel("Введите дополнительные данные работника");
        ...
        JLabel firmAddressLabel = new JLabel("Адрес фирмы");
        ...
        JTextField firmAddressField = new JTextField(15);
        firmAddressField.setFont(new Font(firmAddressField.getFont().getName(), Font.PLAIN, 16));
        layout.putConstraint(SpringLayout.NORTH, firmAddressField, 10, SpringLayout.SOUTH, firmAddressLabel);
        layout.putConstraint(SpringLayout.WEST, firmAddressField, 20, SpringLayout.WEST, jPanel);
        jPanel.add(firmAddressField);

        JLabel firmNameLabel = new JLabel("Название фирмы");
        firmNameLabel.setFont(new Font(firmNameLabel.getFont().getName(), Font.BOLD, 16));
        layout.putConstraint(SpringLayout.NORTH, firmNameLabel, 20, SpringLayout.SOUTH, firmAddressField);
        layout.putConstraint(SpringLayout.WEST, firmNameLabel, 20, SpringLayout.WEST, jPanel);
        jPanel.add(firmNameLabel);

        JTextField firmNameField = new JTextField(15);
        firmNameField.setFont(new Font(firmNameField.getFont().getName(), Font.PLAIN, 16));
        layout.putConstraint(SpringLayout.NORTH, firmNameField, 10, SpringLayout.SOUTH, firmNameLabel);
        layout.putConstraint(SpringLayout.WEST, firmNameField, 20, SpringLayout.WEST, jPanel);
        jPanel.add(firmNameField);

        JButton confirm = new JButton("Добавить работника");
        ...
        confirm.addActionListener(e->{
            ...
            currValues.add("'" + firmAddressField.getText() + "'");
            currValues.add("'" + firmNameField.getText() + "'");

            try {
                String sql = "insert into WORKERS values (" + tableController.getTableSet().getValueAt(
